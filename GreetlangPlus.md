...
GreetLang+ is a bit different. You start by using the `def` keyword. This stops any confusion you would have in C like `int` and `char` and `char varname[100]` (<-- Character array, I'll go in to how that's different in GreetLang+ soon), so it's just `define`. Nothing confusing. <br />
Following that is `var`, which tells the GL+ compiler that we're defining a `VARIABLE`, not something else. Next up is the variable name, and it's value. 
